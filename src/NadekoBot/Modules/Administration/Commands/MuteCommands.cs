        ...
        public class MuteCommands : NadekoSubmodule
        {
            ...
            static MuteCommands()
            {
                ...
                foreach (var conf in configs)
                {
                    foreach (var x in conf.UnmuteTimers)
                    {
                        ...
                        StartUnmuteTimer((ulong) conf.GuildId, (ulong) x.UserId, after);
                    }
                }
            ...
            public static async Task MuteUser(IGuildUser usr)
            {
                ...
                using (var uow = DbHandler.UnitOfWork())
                {
                    ...
                    config.UnmuteTimers.RemoveWhere(x => x.UserId == (long) usr.Id);
            ...
            private static void RemoveUnmuteTimerFromDb(ulong guildId, ulong userId)
            {
                using (var uow = DbHandler.UnitOfWork())
                {
                    ...
                    config.UnmuteTimers.RemoveWhere(x => x.UserId == (long) userId);
                    uow.Complete();
                }
            }
